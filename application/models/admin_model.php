...
class Admin_model extends CI_Model {
    ...
    function get_user_accounts() {
        ...
        $this->flexi_auth->sql_order_by('user_accounts.uacc_date_added', 'DESC');
...
//        }
    ...
    function update_user_account($user_id) {
        ...
        $validation_rules = array(
            array('field' => 'update_name', 'label' => 'Name', 'rules' => 'required'),
        ...
        if ($this->form_validation->run()) {
            ...
            $profile_data = array(
                'upro_id' => $user_id,
                'upro_name' => $this->input->post('update_name'),
                'upro_phone' => $this->input->post('update_phone_number'),
                'upro_nomor_induk' => $this->input->post('update_nomor_induk'),
    ...
    function update_active_user() {
        // Delete groups.
        if ($active_users = $this->input->post('update')) {
            foreach ($active_users as $user_id => $active) {
        ...
        // Redirect user.
        redirect('admin/list_user_status/active');
    }
    
    function update_inactive_user() {
        // Delete groups.
        if ($active_users = $this->input->post('update')) {
            foreach ($active_users as $user_id => $active) {
                $this->flexi_auth->activate_user($user_id);
//                $response = $this->flexi_auth->resend_activation_token($this->flexi_auth->get_user_identity());
            }
        }
        ...
        // Redirect user.
        redirect('admin/list_user_status/inactive');
    }
